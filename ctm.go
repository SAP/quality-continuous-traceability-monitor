...
func main() {
	...
	if *argExportRequirementsMapping {
		projectmanagement.CreateRequirementsMappingReport(cfg.OutputDir+string(os.PathSeparator)+reportBaseName+"_requirements-mapping.json", traces, cfg)
	}
	...
	if cfg.Delivery.Backlogitems != "" {
		...
		if *argExportRequirementsMapping {
			projectmanagement.CreateRequirementsMappingReport(cfg.OutputDir+string(os.PathSeparator)+reportBaseName+"_"+cfg.Delivery.Version+"_requirements-mapping.json", deliveryTraces, cfg)
		}
	}
